...
from src.lw.furniture_ext import FurnitureTaskExtMixin  # [lw]
from src.tasks.NTEOneTimeTask import NTEOneTimeTask


class FurnitureTask(FurnitureTaskExtMixin, NTEOneTimeTask, BaseCombatTask):  # [lw]
    ...
    def claim_anomaly_furniture(self):
        ...
        return self.lw_claim_anomaly_furniture()  # [lw]
    ...
    def claim_furniture(self, furniture):
        if not self.teleport_to_furniture(furniture):
            self.lw_record_furniture_claim_failure(  # [lw]
                "未在房产列表中找到目标家具, 或未能传送到对应房产"
            )
            return False
        ...
        if furniture == Labels.anomaly_fluff:
            self.operate_click(confirm, after_sleep=0.5)
        else:
            self.click_furniture(furniture)
