...
- 本项目新增或重构 LW 自定义逻辑时，优先放在 `src/lw/`，通过 mixin、适配器或接线复用 RU 能力。
- 只有确实无法在 `src/lw/` 内实现时才直接修改 RU 代码；改动应保持最小，并在新增或保留的 LW 代码块、接线和注释处明确添加 `[lw]` 标记，方便后续同步上游时识别。
- 删除已经退役的 LW 代码、恢复 RU 原始实现时无需为了标记而引入新代码。

## 智能体约定同步

- `AGENTS.md` 是 Codex 的项目指令，`CLAUDE.md` 是 Claude 的项目指令；两者共享的仓库约定必须保持同步。
- 修改任一文件中的共享约定时，必须同时检查并更新另一文件，避免 Codex 与 Claude 执行标准不一致。
...
每个可以独立交付的功能或修复，在开发完整且相关测试通过后应直接创建独立 Git 提交，
不等待积累到后续任务。提交时只暂存该功能明确涉及的文件或 hunk，不夹带用户已有改动、
本地配置、日志、截图和无关未跟踪文件。
