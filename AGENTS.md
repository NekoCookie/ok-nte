...
- `# [lw]` 标记表示需要保留的 LW 行为意图, 而不是本地旧代码优先。上游同一区域有更新时, 必须以最新 RU 为底稿重新表达该行为, 并逐项检查标记为“本方法被大幅改写”的区域。
- `src/lw/` 中的整块逻辑通过 mixin、适配器或注册接线复用 RU。用户自有的完整功能文件可以留在其所属领域目录, 但不得把可拆出的 LW 逻辑扩散进 RU 文件。
- Mixin 不用于覆盖 RU 类中已有方法。小范围行为调整可在 RU 方法内保留最小的 `[lw]` 接线; 较大调整应先提取稳定扩展点, 再由 `src/lw/` 实现。生产路径只能保留组合后的新实现。
- 不新增 `LW_*` A/B 开关, “RU 原版 + LW 版”双实现, 旧接口别名或旧异常兼容层。短期诊断对照必须由用户明确要求, 并在验证后删除。
- LW 实例状态在对应 mixin 的 `__init__` 中初始化, 并通过 RU 的 `super().__init__()` 链接入; 不把该状态铺进 RU 的构造函数。
- RU 修改接口、异常或状态模型时, 同一交付必须逐个迁移受影响的 `src/lw/` 导入、调用、捕获和状态处理, 以当前公开契约和回归测试证明迁移完成。
- 归因任何“删除”或“覆盖”前, 必须检查合并基点 `B`、本地父提交 `L`、上游父提交 `U` 和合并结果 `M`。只有路径存在于 `B` 且在 `U` 中被移除, 才能称为上游删除; `B` 中不存在而只存在于 `L` 的路径是本地独有代码, `U` 的缺失不能归因于上游。若 `M` 改写或移除该路径, 必须记录为本地合并决策并逐项审计。使用 `tools/audit_merge_provenance.py <merge>` 生成该证据。
...
- 测试应断言用户可见结果、状态变化、持久化内容、抛出的异常或必要的外部交互; 不要断言普通日志的精确文案或仅为实现细节的调用顺序。只有日志被监控、脚本或支持流程作为稳定契约消费时，才测试其内容。
